...
def parse_pdf(file_path: str):
    ...
    Falls back to pdfplumber for pages that yield no text from pypdf (e.g. image-heavy pages).
    """
    ...
    with logfire.span("PDF parsing (local", filename=file_path):
        try:
            ...
            # Fallback: use pdfplumber for any pages that pypdf returns blank
            if blank_pages:
            ...
            logfire.error(f"Failed to parse PDF for {file_path}: {e}")
            raise e
